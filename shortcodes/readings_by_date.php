<?php
//cmog_readings-by-date   
 
add_shortcode( 'readings_by_date', 'cmog_readingsbydate' );
function cmog_readingsbydate(){
...
$outputcal .= "<h3>Readings $SMonth/$SYear </h3>" .PHP_EOL;
  if ($Nativity['wday'] <> 6) {
...
// ECHO ("<small><br>" . mysql_num_rows ($result) . " entries found <br></small><ul>");
	//$outputcal .=  
 $data = $wpdb->get_results( "SELECT * FROM cmog66_cmog_events  where Class = 'read'  and Month = $SMonth and (Year = -1 or Year = $SYear) ORDER BY Month , Day "); 
 $lastEventDay = "";
