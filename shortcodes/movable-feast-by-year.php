...
$outputcal = '';
$Y_year = (!empty ($_REQUEST['f_year'] )) ? $_REQUEST['f_year'] : '';
...
<?php $outputcal .= "<h3>Movable Feast by Year " . $Y_year . "</h3>";?> 
<?php
...
$P_date = getDate(mktime(0, 0, 0, 3 ,$RC + 34 , $Y_year));
$outputcal .= "<h3>$P_date[year] Movable Feast Dates </b></h3>" ; 


$outputcal .= "<FORM ACTION='#' METHOD=GET>Change Year: <INPUT type='text' name='Year' VALUE='$Y_year' size='4'>" ;
$outputcal .= "<INPUT TYPE=SUBMIT NAME='Change Year' VALUE='Enter'> </FORM>" ;
$outputcal .= "<hr>" ;

//
//-----------------------------------------------------------------
$outputcal .= "<hr />" ;
 $outputcal .= "From Pentecost of " . ($Y_year - 1) . "  to Pre-Lent of " . $Y_year . " is " .  'Weeks_of_Pentecost($P_date - 1)' . "  weeks <br>" ;
$outputcal .= "<hr />" ;
...
$outputcal .= "<small>(Theophany is on a $T_date[weekday] this year.</small>" ; 
$SBT_date = getDate(mktime(0, 0, 0, 1,(6 - $T_date['wday'])  , $Y_year));

if  (($T_date['wday'] == 0) or ($T_date['wday'] == 6)) {
$outputcal .= "<small> Sunday before Theophany would be last year)</small>" ; 
      }
$outputcal .= "<ul>" ;      
if  ($T_date['wday'] <> 0) {
     $outputcal .= process_date ($T_date,(-$T_date['wday']),'Sunday before Theophany. ', 'lf', '' ,  1 , '' , ''); 
     $outputcal .= process_date ($T_date,((-$T_date['wday'])+ 7),'Sunday after Theophany ', 'lf', '' ,  1 ,'' , ''); 
     } else {
     $outputcal .= process_date ($T_date,-7,'Sunday before Theophany', 'lf', '' ,  1 ,'' , '') ; 
     $outputcal .= process_date ($T_date,+7,'Sunday after Theophany ', 'lf', '' ,  1 ,'' , '') ; 
     }
$outputcal .= "</ul><hr />" ;
...
$outputcal .=       process_date($P_date,$offset, $eventText, $class , $link, $length, $hymn, $icon, $listorder); 
   }// for each end
...
$outputcal .= "(No Fast of the Holy Apostles this year.)<br>" ;
      } else {
$outputcal .= "<ul>" ;
$outputcal .=  process_date($P_date,+57,'Apostles fast begins', 'fast'); 

      if (($f_date['yday'] + 1) < $a_date['yday']) { //more than one day fast//
			 
$outputcal .=  process_date($P_date,+58, 'Apostles fast','fast',NULL,($a_date['yday'] - $f_date['yday'] -1)) ; 
       } //more
$outputcal .= "</ul>" ;
      }  
...
if  ($L_date['wday'] <> 0) {
$outputcal .= process_date ($L_date,(-$L_date['wday']),'Sunday before the Elevation','lf');
                   } else {
$outputcal .= process_date ($L_date,(-7),'Sunday before the Elevation','lf');
                   }
$outputcal .= process_date ($L_date,(7 - $L_date['wday']),'Sunday after the Elevation','lf');
// Monday after the Sunday after the Elevation
$outputcal .= process_date ($L_date,(8 - $L_date['wday']),'Lukan Jump','lf');
...
$outputcal .= "<small>(Nativity is on a $N_date[weekday] this year)</small>" ; 
$outputcal .= "<ul>" ;
...
if  ($N_date['wday'] <> 0) {
    ...
    $outputcal .= process_date ($N_date,((-$N_date['wday'])+ 7),'Sunday after Nativity', 'lf', 'http://ocafs.oca.org/FeastSaintsLife.asp?FSID=81' , 1 ,
     ...
     $outputcal .= process_date ($N_date,-7,'Sunday before Nativity', 'lf', 'http://ocafs.oca.org/FeastSaintsLife.asp?FSID=81' , 1 ,
		 'http://www.oca.org/FStropars.asp?SID=13&ID=81' , 'http://saints.oca.org/IconDirectory/XSM/nativity/1225nativity15.jpg'); 
$outputcal .= "<small>(Sunday after Nativity would be next year so readings are done on 12/26/" . $Y_year . ")</small>" ; 
...
//-----------------------------------------------------------------
$outputcal .= "</ul><hr />" ;
$outputcal .= "From Pentecost of " . $Y_year . "  to Pre-Lent of " . ($Y_year + 1) . " is " . "Weeks_of_Pentecost($Y_year)" . "  weeks " ;
$outputcal .= "<hr />" ;
...
$WL = $WI - $WJ;
$WEM = 3 + int_div((0 + 40) , 44 ); // Western Easter month
$WED = $WL +28 - (31 * int_div($WEM , 4)); //Western Easter day
 $Westerm_Easter = getDate(mktime(0, 0, 0, $WEM, $WED, $Y_year));
$outputcal .= "<hr /><b>(Westerm Easter is $Westerm_Easter[mon]/$Westerm_Easter[mday]/$Y_year)</b><hr />" ;
$outputcal .= "<small>Note that dates are on the Gregorian calendar, even for dates calculated on the Julian calendar and for years before the Gregorian calendar was adopted. Also no corections were made for years after 2099 yet.</small><br /><small>CMOG-Calendar</small>";
RETURN $outputcal;
}