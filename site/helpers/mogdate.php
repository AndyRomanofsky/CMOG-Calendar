<?php
/**
 * Joomla! 1.5 component MOG Calendar
 *
 * @version $Id: helper.php 2011-05-19 13:46:47 svn $
 * @author Andrew Romanofsky
 * @package Joomla
 * @subpackage CMOGCAL (from MOG Calendar)
 * @license GNU/GPL
 *
 * Church Calendar Component
 *
 *
 */
...
//  this Function returns the Number of week from Pentecost to the Pre-lent season.

//

//  Input:   $C_year                       - four digit year from 1906 to 2036)  

//  retures count of weeks

//

//  uses     $PDate[Pen_date]              - timestamp of Pentecost for input date.
...
//         $I_day     - day of month
//         $return_array  - flag to the $Pentecost_day_week aray:
//
//         $Pentecost_day_week[day]  - 8 is Sunday , 1 is Mon, 2 is Tues, ect.
...
//
//        otherwise test is returned

function Pentecost_offset($I_year, $I_month, $I_day, $return_array = false) {
...
//newend	
	...
	if ($return_array) {
	 ...
	 } elseif ($Pentecost_day_week[week_of_Pascha] == 1 ){
...
//          $Pentecost_day_week[week_of_Pascha] -  week number of readings form Pascha to Pentecost
//
function Echo_Pentecost_offset($I_year, $I_month, $I_day) {
...
echo ("<br>$day_name, $month_name  $I_day, $I_year<br>"); 
...
if (($Pascha_date[yday] - 16) == $March25[yday]) 	$Pentecost_day_week[Annunciation] = 1;
 ...
 $week_after_Pentecost = (floor((($I_date[0] -  $Pentecost_lastyear[0])) / 604800));		
  ...
  elseif (($Pascha_date[yday] - $I_date[yday]) < 7) : echo("$day_name, Holy Week<br>Triodion week 10<br>"); 
	                              $Pentecost_day_week[holyweek] =1;
	                              $Pentecost_day_week[week_of_Triodion] = 10;
  elseif (($Pascha_date[yday] - $I_date[yday]) == 7) : echo( "Entry of Our Lord into Jerusalem<br>Triodion week 10<br>");
	                              $Pentecost_day_week[holyweek] =1;
	                              $Pentecost_day_week[week_of_Triodion] = 10;
  elseif (($Pascha_date[yday] - $I_date[yday]) == 8) : echo( "Lazarus Saturday<br>Triodion week 9<br>");  
	                              $Pentecost_day_week[week_of_Triodion] = 9;
...
//$lookup .= "<br>Look up Triodion reading week " . $ChurchDates[week_of_Triodion] . " day " . $ChurchDates[day]; 
      $result = mysql_query("SELECT * FROM jo25_cmog_templates WHERE ((`week` = '$ChurchDates[week_of_Triodion]') and (`wday` = '$ChurchDates[day]') and (`gmd` = -4))
...
//$lookup .= "<br>Look up Pascha reading week " . $ChurchDates[week_of_Pascha] . " day " . $ChurchDates[day]; 
      $result = mysql_query("SELECT * FROM jo25_cmog_templates WHERE ((`week` = '$ChurchDates[week_of_Pascha]') and (`wday` = '$ChurchDates[day]' ) and (`gmd` = -5))
             ORDER BY `listorder` DESC ");
...
if ($ChurchDates[day] ==  0) $ChurchDates[day] = 7;
 //$lookup .= "<br>Look up Pentecost reading week " . $ChurchDates[week] . " day " . $ChurchDates[day]; 
     $result = mysql_query("SELECT * FROM jo25_cmog_templates WHERE ((`week` = '$ChurchDates[week]') and (`wday` = '$ChurchDates[day]') and (`gmd` = -2))
             ORDER BY `listorder` DESC ");
  ...
  if  ($ChurchDates[lukew] <> 0) {
      //$lookup .= " and for Luke reading week " . $ChurchDates[lukew] . " day " . $ChurchDates[day]; 
           $result2 = mysql_query("SELECT * FROM jo25_cmog_templates WHERE ((`week` = '$ChurchDates[lukew]') and (`wday` = '$ChurchDates[day]' ) and (`gmd` = -3))
             ORDER BY `listorder` DESC ");
