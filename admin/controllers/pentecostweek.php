...
/**
 * PentecostWeek Controller
 */
class CmogCalControllerPentecostWeek extends JControllerForm
{
    /**
    ...
    protected function allowEdit($data = array(), $key = 'ID')
    {
        ...
        if( !empty( $id ) ){
            ...
            return $user->authorise( "core.edit", "com_cmogcal.pentecostweek." . $id );
        }
    }
		...
		public function batch($model = null)
		{
			...
			// Set the model
			$model = $this->getModel('pentecostweek', '', array());
	
			// Preset the redirect
			$this->setRedirect(JRoute::_('index.php?option=com_cmogcal&view=pentecostweeks' . $this->getRedirectToListAppend(), false));
		...
		// Pentecost date for filter year
		$pt_date = getDate(mktime(0, 0, 0, $pa_date[mon], ($pa_date[mday] + 49), $pa_date[year]));
			...
			//
			//calulate 	date
			$load_date = getDate(mktime(0, 0, 0, $pt_date[mon], ($pt_date[mday] + $row[wday] +(($row[week] -1)*7)), $pt_date[year]));
