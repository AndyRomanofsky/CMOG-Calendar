 ...
 * LukeWeek Controller
 */
class CmogCalControllerLukeWeek extends JControllerForm
{
    /**
    ...
    protected function allowEdit($data = array(), $key = 'ID')
    {
        ...
        if( !empty( $id ) ){
            ...
            return $user->authorise( "core.edit", "com_cmogcal.lukeweek." . $id );
        }
    }
	...
	public function batch($model = null)
	{
		...
		// Set the model
		$model = $this->getModel('lukeweek', '', array());

		// Preset the redirect
		$this->setRedirect(JRoute::_('index.php?option=com_cmogcal&view=lukeweeks' . $this->getRedirectToListAppend(), false));
...
//
//calulate 	date
$load_date = getDate(mktime(0, 0, 0, $sal_date[mon], ($sal_date[mday] + $row[wday] +(($row[week] -1)*7)), $sal_date[year]));
