...
/**
 * MoveableEvent Controller
 */
class CmogCalControllerMoveableEvent extends JControllerForm
{
    /**
    ...
    protected function allowEdit($data = array(), $key = 'ID')
    {
        ...
        if( !empty( $id ) ){
            ...
            return $user->authorise( "core.edit", "com_cmogcal.moveableevent." . $id );
        }
    }
	...
	public function batch($model = null)
	{
		...
		// Set the model
		$model = $this->getModel('moveableevent', '', array());

		// Preset the redirect
		$this->setRedirect(JRoute::_('index.php?option=com_cmogcal&view=moveableevents' . $this->getRedirectToListAppend(), false));
...
$p_date= MOGDate::getDate_of_Pascha($filter_year);
	...
	// Read a template
	// 
	$result = mysql_query('SELECT * from jo25_CMOG_MoveableEvent where ID = '. $tmplt_id );
	 $row = mysql_fetch_array($result);
	//
	//calulate 	date
	$load_date = getDate(mktime(0, 0, 0, $p_date[mon], ($p_date[mday] + $row[Offset] ), $p_date[year]));
  ...
  $load_date = getDate(mktime(0, 0, 0, $LMonth,($LDay + $rep_count), $LYear));
            $rep_count = $rep_count + 1;
	//check that any are not already loaded
	$result = mysql_query('SELECT ID from jo25_CMOG_Events where gmd = ' . $row["gmd"] . ' and Year = ' . $load_date[year] . ' and Month = ' . $load_date[mon] . ' and Day = ' . $load_date[mday] . ' and tmplt_id = '. $tmplt_id );
	...
	$this->loadone($tmplt_id,$load_date[year],$load_date[mon],$load_date[mday]);	
		}
				} //while count
			}//while cid
		...
		/**
		 *
		 *  load template   NOT USED YET
		 */
		 
		  public function xxxxxload()
		 {
	...
	$result = mysql_query($SQL);
			...
			$result = mysql_query('SELECT * from jo25_CMOG_MoveableEvent where ID = '. $tmplt_id );
			if (!$result) {
			 JFactory::getApplication()->enqueueMessage( "Error reading template: " .  mysql_error() , 'Warning');	
				} else {
			 $row = mysql_fetch_array($result);
  ...
  $load_date = getDate(mktime(0, 0, 0, $LMonth,($LDay + $rep_count), $LYear));
			...
			$SQLvalues .= $tmplt_id . "', '" . $load_date[year] . "', '" . $load_date[mon] . "', '" . $load_date[mday] . "') ";
			...
			JFactory::getApplication()->enqueueMessage(JText::_('Event - '  . $load_date[mon] . "/" . $load_date[mday] . "/"  .$load_date[year] . " - " .$row["EventText"] .' loaded from template.'), 'Message');
				 }
			  }   //while
			}
					// Preset the redirect
