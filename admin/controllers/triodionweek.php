...
/**
 * TriodionWeek Controller
 */
class CmogCalControllerTriodionWeek extends JControllerForm
{
    /**
    ...
    protected function allowEdit($data = array(), $key = 'ID')
    {
        ...
        if( !empty( $id ) ){
            ...
            return $user->authorise( "core.edit", "com_cmogcal.triodionweek." . $id );
        }
    }
		...
		public function batch($model = null)
		{
			...
			// Set the model
			$model = $this->getModel('triodionweek', '', array());
	
			// Preset the redirect
			$this->setRedirect(JRoute::_('index.php?option=com_cmogcal&view=triodionweeks' . $this->getRedirectToListAppend(), false));
...
$st_date = getDate(mktime(0, 0, 0, $pa_date[mon], ($pa_date[mday] - 70), $pa_date[year]));
		...
		//
		//calulate 	date
		if ($row[week] < 0) $row[week] = $row[week] + 1; // no week 0
		$load_date = getDate(mktime(0, 0, 0, $st_date[mon], ($st_date[mday] + $row[wday] +(($row[week] -1)*7)), $st_date[year]));
